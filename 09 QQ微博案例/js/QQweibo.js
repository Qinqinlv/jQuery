...
* @Date:   2017-04-01 17:02:50
* @Last Modified by:   Administrator
* @Last Modified time: 2017-04-01 17:44:39
*/

'use strict';

$(function(){
	//要实现的效果是，当鼠标放在“表情”这个超链接上的时候，会出现一个盒子，
	//这个盒子中存放的是各种表情，并且鼠标滑动到这个盒子上可以选择表情，
	//被选择表情对应的文字内容会出现在输入框中。解题思路如下：
	
	//1.  在加载的时候动态生成一个div存放所有表情，默认隐藏
	...
	var $divFace = $("<div class='showFace'></div>")
	for(var key in userFaces){
		var $img = $("<img src='faces/"+key+"' alt='"+userFaces[key]+"'>");
		$divFace.append($img);
	}
	$divFace.hide();
	//2.  当鼠标移动到“表情”超链接上的时候，显示该盒子，鼠标移开的时候隐藏该盒子
	$(".insertFace").append($divFace).hover(function(){
		$divFace.show();
	},function(){
		$divFace.hide();
	})
	//当点击盒子上的任意一个表情时，该标签的文字会显示到输入框中
	$divFace.find("img").on("click",function(){
		var setTxt = $(this).attr("alt");
		$("#msgTxt").text($("#msgTxt").text() + "["+setTxt+"]" );
	})
